...
---@field pos Vector
---@field radius number
---@field base_radius number
---@field speed number
---@field dead boolean
---@field hue number
---@field focus number 0 to 1
---@field is_focusing boolean
---@field timer Timer
---@field ring_timer number
---@field update fun(self: Player, dt: number, keys: table<string, boolean>, spawn_ring: fun(r: Ring))
...
---@param spawn_ring fun(r: Ring)
function Player.update(self, dt, keys, spawn_ring)
	...
	if self.is_focusing ~= was_focusing then
		local target_radius = self.is_focusing and 20 or self.base_radius
		self.timer:clear()
	...
	self.pos = self.pos + move * current_speed * dt
