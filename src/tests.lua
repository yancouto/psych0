...
local function runAllLevels()
	...
	for _, levelName in ipairs(levels) do
		if levelName == "__polyfill__.lua" or not levelName:match "%.lua$" then goto continue end

		local levelPath = "levels/" .. levelName
		print(COLOR_YELLOW .. "Testing level: " .. levelPath .. COLOR_RESET)

		local content = love.filesystem.read("string", levelPath) --[[@as string?]]
		if not content then error("Could not read level file: " .. levelPath) end

		local success, err = pcall(function()
			local instance = interpreter.runLevel(content, {
				spawnCallback = function() end,
				spawnIndicatorCallback = function() end,
				getEnemyCount = function() return 0 end,
			})
			-- Run until finished or some limit to avoid infinite loops if script has bugs
			local iters = 0
			while not instance:update(1) and iters < 10000 do
				iters = iters + 1
			end
		end)
